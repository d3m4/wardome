...
void zedit_setup(struct descriptor_data *d, int room_num)
{
  ...
  zone->owner = zone_table[OLC_ZNUM(d)].owner;
  /*
...
void zedit_save_internally(struct descriptor_data *d)
{
  ...
  if (OLC_ZONE(d)->number) {
    ...
    zone_table[OLC_ZNUM(d)].owner = OLC_ZONE(d)->owner;
  }
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MAIN_MENU:
    ...
    for(i=0; i < numero_clans; i++)
      ...
      OLC_MODE(d) = ZEDIT_ZONE_OWNER;
      break;    
    }
/*
       send_to_char("Enter Clan Name: ", d->character);
       OLC_MODE(d) = ZEDIT_ZONE_OWNER;
       break;
*/  
    case 't':
...
/*-------------------------------------------------------------------*/
    case ZEDIT_ZONE_OWNER:
    /*
     *  Change owner of zone
     */ 
    pos = atoi(arg);
      OLC_ZONE(d)->owner = pos;
      OLC_ZONE(d)->number = 1;
      zedit_disp_menu(d); 
  break;
/*----------------------------------------------------------------------*/  
