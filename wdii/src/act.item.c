...
*   File: act.item.c                                                      *
*                                                                         *
*  karl: isto era parte do CircleMUD 3.1 mas eu tive que copiar/colar/mod *
*  o arquivo original para conseguir compilar o Wardome.                  *
*  Em virtude deste hack vocês tem que consertar as mensagens impressas   *
*  pelo send_to_char()                                                    *
*                                                                         *
************************************************************************ */
...
ACMD(do_put)
{
  ...
  if (!*theobj)
    ...
    send_to_char("What do you want to put it in?\r\n", ch);
  } else {
    generic_find(thecont, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &tmp_char, &cont);
    if (!cont)
      send_to_char("You don't see SOMETHING SOMETHING here.\r\n", ch);
	...
	if (!(obj = get_obj_in_list_vis(ch, theobj, ch->carrying)))
	  send_to_char("You aren't carrying SOMETHING SOMETHING.\r\n", ch);
	...
	if (!found) {
	  if (obj_dotmode == FIND_ALL)
	    ...
	    send_to_char("You don't seem to have any %ss.\r\n", ch);
	}
      }
    }
  }
...
void get_from_room(struct char_data *ch, char *arg, int howmany)
{
  ...
  if (dotmode == FIND_INDIV) {
	...
	send_to_char("You don't see any SOMETHINGS here.\r\n", ch);
    }
  }
}
...
ACMD(do_get)
{
  ...
  if (!*arg1)
    ...
    if (cont_dotmode == FIND_INDIV) {
      ...
      if (!cont)
	send_to_char("You don't have SOMETHING SOMETHING.\r\n", ch);
      else if (GET_OBJ_TYPE(cont) != ITEM_CONTAINER)
	act("$p is not a container.", FALSE, ch, cont, 0, TO_CHAR);
      else
	...
	if (cont_dotmode == FIND_ALL)
	  ...
	  send_to_char("You can't seem to find any SOMETHING here.\r\n", ch);
      }
    }
  }
}
...
*/
  ...
  if (!*arg) {
    send_to_char("What do you want to SOMETHING?\r\n", ch);
    return;
    ...
    if (!str_cmp("coins", arg) || !str_cmp("coin", arg))
      ...
      send_to_char("What do you want to SOMETHING SOMETHINGd of?\r\n", ch);
    else if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying)))
      send_to_char("You don't seem to have any SOMETHING.\r\n", ch);
    else {
      do {
	...
	for (obj = ch->carrying; obj; obj = next_obj) {
      ...
      if (!*arg) {
	send_to_char("What do you want to SOMETHING all of?\r\n", ch);
	return;
      }
	...
	obj = next_obj;
      ...
      if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying)))
	send_to_char("You don't seem to have SOMETHING SOMETHING.\r\n", ch);
      else
	amount += perform_drop(ch, obj, mode, sname, RDR);
    }
  }
...
ACMD(do_give)
{
  ...
  if (!*arg)
	...
	perform_give_gold(ch, vict, amount);
      ...
      send_to_char("What do you want to give AMOUNT of?\r\n", ch);
    else if (!(vict = give_find_vict(ch, argument)))
      return;
    else if (!(obj = get_obj_in_list_vis(ch, arg, ch->carrying))) 
      send_to_char("You don't seem to have any ARG.\r\n", ch);
    else {
      while (obj && amount--) {
	next_obj = get_obj_in_list_vis(ch, arg, obj->next_content);
...
ACMD(do_eat)
{
  ...
  if (!(food = get_obj_in_list_vis(ch, arg, ch->carrying))) {
    send_to_char("You don't seem to have AN(arg), arg.\r\n", ch);
    return;
  }
...
ACMD(do_pour)
{
  ...
  if (subcmd == SCMD_FILL) {
    ...
    if (!(from_obj = get_obj_in_list_vis(ch, arg2, world[IN_ROOM(ch)].contents))) {
      send_to_char("There doesn't seem to be AN(arg2), arg2 here.\r\n", ch);
      return;
    }
...
ACMD(do_wear)
{
  ...
  if (dotmode == FIND_ALL) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg1, ch->carrying)))
      send_to_char("You don't seem to have any arg1.\r\n", ch);
    else
      while (obj) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg1, ch->carrying)))
      send_to_char("You don't seem to have AN(arg1) arg1.\r\n", ch);
    else {
  ...
  if (dotmode == FIND_ALL) {
	...
	send_to_char("You don't seem to be using any arg.\r\n", ch);
      ...
      if ((i = get_obj_pos_in_equip_vis(ch, arg, NULL, ch->equipment)) < 0)
        send_to_char("You don't seem to be using.\r\n",ch);
      else
        perform_remove(ch, i);
